...
//         g_Battle.UI.iPrevEnemyTarget = g_Battle.UI.iSelectedIndex; //disabled due to not same as both original version
   ...
   for (i = 0; i < BATTLEUI_MAX_SHOWNUM; i++)
   {
      ...
      {
         //if ((SDL_GetTicks() - g_Battle.UI.rgShowNum[i].dwTime) / (BATTLE_FRAME_TIME) > 100000000000)
         //{
         ...
         //else
         {
            PAL_DrawNumber(g_Battle.UI.rgShowNum[i].wNum, 5,
               PAL_XY(PAL_X(g_Battle.UI.rgShowNum[i].pos), PAL_Y(g_Battle.UI.rgShowNum[i].pos) - (SDL_GetTicks() - g_Battle.UI.rgShowNum[i].dwTime) / 10),
