...
class IFU extends Module with HasResetVector{
  ...
  state := MuxLookup(state, addr, List(
    ...
    data    -> Mux(dataFire && io.lsuReady, addr, data) // 当lsu阶段执行完后才回到addr状态，开始取指,todo
  ))
  ...
  pc                      := MuxLookup(state, pc, List(
                              ...
                              data  -> Mux(!(dataFire && (!io.stall || io.stall && io.lsuReady)), pc, Mux(io.axiDataIO.bits.rresp === okay, Mux(io.redirect.valid, dnpc, snpc), pc))
                            ))
