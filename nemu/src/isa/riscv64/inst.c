...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  __VA_ARGS__ ; \
	...
	// 模式匹配宏，nemu/include/cpu/decode.h
	// INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
	// 如果匹配到指令，则会调用decode_operand()
  INSTPAT_START();
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2);
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(dest) = (int32_t)(src1 * src2));
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = s->pc + imm);
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(dest) = imm);
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, R(dest) = Mr(src1 + imm, 8));
	INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + imm);
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = (int32_t)(src1 + imm));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, R(dest) = s->pc + 4; s->dnpc = (src1 + imm) & 0xfffffffffffffffe);
...
int isa_exec_once(Decode *s) {
	// 执行inst_fetch后snpc的值才变为pc+4, 否则就是当前的pc 
